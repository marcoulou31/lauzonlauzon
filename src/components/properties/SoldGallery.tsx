...
export function SoldGallery({ photos }: Props) {
    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {photos.map((photo, index) => (
          <figure key={photo.src} className="group flex flex-col">
            <button
              type="button"
              onClick={() => open(index)}
              className="relative aspect-4/3 w-full overflow-hidden bg-cream-dark cursor-pointer"
            ...
            <figcaption className="mt-3 text-sm leading-relaxed text-navy/70">
              {photo.caption}
            </figcaption>
          </figure>
        ))}
