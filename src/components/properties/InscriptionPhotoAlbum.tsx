...
import type { InscriptionPhoto } from "@/lib/types";
...
export function InscriptionPhotoAlbum({ photos, address }: Props) {
  ...
  // Deux frames apres l'ouverture -> on declenche la transition d'entree.
  useEffect(() => {
    if (!isOpen) return;
  ...
  // Fin de la transition de sortie -> demontage reel.
  useEffect(() => {
  ...
  // Garde la miniature active visible (centree) dans la bande de miniatures.
  useEffect(() => {
    if (!isOpen) return;
  ...
  const activeAlt = activePhoto.piece ?? address;
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {photos.map((photo, index) => (
          ...
          >
            ...
            {photo.piece && (
              <span className="absolute bottom-0 left-0 right-0 bg-navy/70 text-white text-xs px-2 py-1 text-center">
                {photo.piece}
              </span>
            )}
          </button>
              ...
              <div className="bg-white shadow-2xl overflow-hidden">
                ...
                <div className="flex items-center justify-between gap-3 border-b border-cream-dark px-5 py-3">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-navy">{activeAlt}</p>
